...
from pinned versions. `requirements.txt` holds what is needed to *run* the pipeline
and the app; `requirements-dev.txt` adds the test, notebook and screenshot tooling.
...
├── data/samples/                 5 MB slice of the transactions, tracked so the hosted demo works
├── notebooks/
...
`make all`. Two deliberate exceptions are tracked so the app runs straight from a
clone: `data/samples/creditcard_sample.parquet` (the most recent 20,000 transactions,
5.4 MB, 12 frauds) and the autoencoder with its scaler (~660 KB). The 73 MB raw
dataset and the 10 MB Isolation Forest stay out.

## Deploying the demo

The app is built to run on nothing but what the repository contains. The synthetic
ERP extract is generated in memory on each cold start (0.4 s, same seeds as the
published metrics) rather than read from disk, and the transaction batch comes from
the tracked sample. Anything missing degrades rather than crashes: no Isolation Forest
artifact simply removes that baseline, and an unknown schema falls back to quality
scoring plus a forest fitted on the uploaded batch.

On [Streamlit Community Cloud](https://share.streamlit.io): point it at this repo,
set the entry point to `app/streamlit_app.py` and the Python version to 3.11. The
pinned `requirements.txt` is resolved for both macOS and Linux, and on Linux it
installs `torch==2.14.0+cpu` from the PyTorch CPU index — the default PyPI wheel pulls
in a ~2.5 GB CUDA runtime that exceeds the hosting size limit. Expect a slow first
build while torch installs.
